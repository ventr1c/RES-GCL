...
parser.add_argument('--encoder_model', type=str, default='Grace', help='propagation model for encoder',
                    choices=['Grace','Grace_flip','GraphCL','BGRL','DGI','GAE','Node2vec','Grace-Jaccard'])
...
parser.add_argument('--attack', type=str, default='none',
                    choices=['nettack','random','none', 'PRBCD', 'CLGA'],)
...
from models.random_smooth_flip import sample_noise_all_dense,sample_noise_1by1_dense, _lower_confidence_bound
...
accs = []
if(args.if_smoothed):
    betas = [0.1,0.2,0.01,0.05,0.3,0.4,0.5,0.9,0.001,0.005]
    # betas = [0.3]
else:
    betas = [-1]
for beta in betas:
    ...
    if(args.attack == 'nettack'):
        ...
        perturbation_sizes = [0, 0.01, 0.05, 0.10]
        accuracys = {}
        for n_perturbation in perturbation_sizes:
            accuracys[n_perturbation] = []
    ...
    # In[30]:
        ...
        # Node-level task
        print(args.attack)
        import models.random_smooth_flip as random_smooth_flip
        num_class = int(data.y.max()+1)
        smooth_model = random_smooth_flip.Smooth_Ber(args, model, num_class, args.prob, data.x, if_node_level=True,device=device)
