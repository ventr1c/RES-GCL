...
parser.add_argument('--encoder_model', type=str, default='Grace', help='propagation model for encoder',
                    choices=['Grace','GraphCL','BGRL','DGI','GAE','Node2vec','Grace-Jaccard','LR'])
...
parser.add_argument('--attack', type=str, default='none',
                    choices=['nettack','random','none', 'PRBCD', 'CLGA','random_global'])
...
#     args.drop_edge_rate_1 = 0.6
#     args.drop_edge_rate_2 = 0.3
#     args.drop_feat_rate_1 = 0.2
#     args.drop_feat_rate_2 = 0.3
#     args.tau = 0.2
#     args.cl_lr = 0.01
#     args.weight_decay = 1e-5
#     args.cl_num_epochs = 2000
#     args.num_hidden = 128
# elif(args.dataset == "Photo"):
#     args.drop_edge_rate_1 = 0.3
#     args.drop_edge_rate_2 = 0.5
#     args.drop_feat_rate_1 = 0.1
#     args.drop_feat_rate_2 = 0.1
#     args.tau = 0.3
#     args.cl_lr = 0.1
#     args.weight_decay = 1e-5
#     args.cl_num_epochs = 2000
...
#     args.drop_edge_rate_1 = 0.3
#     args.drop_edge_rate_2 = 0.2
#     args.drop_feat_rate_1 = 0.3
#     args.drop_feat_rate_2 = 0.4
#     args.tau = 0.4
#     args.cl_lr = 0.1
#     args.weight_decay = 1e-5
#     args.cl_num_epochs = 1000
#     args.num_hidden = 256
#     args.num_proj_hidden = 256
...
#     args.cl_lr = 0.01
#     args.weight_decay = 1e-5
#     args.cl_num_epochs = 1500
#     args.num_hidden = 128
#     args.num_proj_hidden = 64
# elif(args.dataset == "WikiCS"):
#     args.drop_edge_rate_1 = 0.5
#     args.drop_edge_rate_2 = 0.5
#     args.drop_feat_rate_1 = 0.1
#     args.drop_feat_rate_2 = 0.1
#     args.tau = 0.1
...
from eval import label_classification,label_evaluation,label_classification_origin, smoothed_linear_evaluation, lr_evaluation
...
for beta in betas:
    ...
    if(args.attack == 'nettack'):
        ...
        for n_perturbation in perturbation_sizes:
            accuracys[n_perturbation] = []
    elif(args.attack == 'CLGA'):
        # perturbation_sizes = [0, 0.05, 0.10]
        ...
        if(args.attack == 'nettack'):
            ...
            for n_perturbation in perturbation_sizes:
                ...
                for target_node in tqdm(target_node_list):
                    ...
                    model = model_construct(args,args.encoder_model,data,device)
                    model.fit(data.x, perturb_edge_index,perturb_edge_weight,data.y,train_iters=args.cl_num_epochs,seen_node_idx=None,verbose=True)
                    model.eval()
                    if(args.if_smoothed):
            ...
            for n_perturbation in perturbation_sizes:
                ...
                model = model_construct(args,args.encoder_model,data,device)
                model.fit(data.x, noisy_data.edge_index,noisy_data.edge_weight,data.y,train_iters=args.cl_num_epochs,seen_node_idx=None,verbose=True)
                model.eval()
                if(args.if_smoothed):
            ...
            # for n_perturbation in perturbation_sizes:
                ...
                if(args.if_smoothed):
                    if(args.dataset == 'WikiCS'):
                        wiki_accs = []
                        for i in range(atk_budget+1):
                            idx_train = data.train_mask[:,i].nonzero().flatten()
                            idx_val = data.val_mask[:,i].nonzero().flatten()
                            idx_clean_test = data.test_mask.nonzero().flatten()
                            prediction_distribution, acc = smooth_model._sample_noise_ber(args.num_sample, noisy_data.edge_index, noisy_data.edge_weight, noisy_data.y, if_node_level = True, idx_test = idx_clean_test, idx_train = idx_train)
                            prediction = prediction_distribution.argmax(1)
                            wiki_accs.append(acc)
                        acc = sum(wiki_accs) / len(wiki_accs)
                    else:
                        prediction_distribution, acc = smooth_model._sample_noise_ber(args.num_sample, noisy_data.edge_index, noisy_data.edge_weight, noisy_data.y, if_node_level = True, idx_test = idx_clean_test, idx_train = idx_train)
                        prediction = prediction_distribution.argmax(1)
                    # acc, prediction = smoothed_linear_evaluation(args, model, noisy_data.x, noisy_data.edge_index, noisy_data.edge_weight, 100, noisy_data.y, idx_train, idx_clean_test, device)
            ...
            for global_ptb_rate in perturbation_sizes:
                ...
                if(global_ptb_rate > 0):
                    # noisy_data = generate_prbcd_attack.generate_prbcd_attack(noisy_data, global_ptb_rate, device)
                    perturbed_adj = pkl.load(open('clga_poisoned_adj/%s_%s_%f_adj.pkl' % (args.dataset, 'CLGA', global_ptb_rate), 'rb')).to(device)
                    noisy_data.edge_index, noisy_data.edge_weight = dense_to_sparse(perturbed_adj)
                    noisy_data.edge_index, noisy_data.edge_weight = noisy_data.edge_index.to(device), noisy_data.edge_weight.to(device)
                model = model_construct(args,args.encoder_model,data,device)
                model.fit(data.x, noisy_data.edge_index,noisy_data.edge_weight,data.y,train_iters=args.cl_num_epochs,seen_node_idx=None,verbose=True)
                model.eval()
                if(args.if_smoothed):
