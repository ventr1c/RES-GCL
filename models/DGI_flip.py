...
class Encoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        if(self.args.if_smoothed==True):
            ...
            edge_index1, edge_weight1  = random_smooth_flip.sample_noise_all_sparse(self.args,edge_index,None, x, self.device)
            # edge_index1, edge_weight1 = random_smooth.sample_noise_all_dense(self.args,edge_index,None,self.device)
...
class DGI(torch.nn.Module):
    def __init__(self, args, nfeat, nhid, nproj, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,tau=None, layer=2,if_smoothed=True,device=None,use_ln=False,layer_norm_first=False):
        ...
        self.gconv = GCN_body(nfeat, nhid, dropout, layer,device=device,use_ln=use_ln,layer_norm_first=layer_norm_first).to(device)
