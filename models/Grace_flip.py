...
from models.random_smooth import sample_noise_all_dense,sample_noise_1by1_dense,sample_noise_all_dense
...
class Grace_flip(nn.Module):
    ...
    def __init__(self, args, nfeat, nhid, nproj, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,tau=None, layer=2,if_smoothed=True,device=None,use_ln=False,layer_norm_first=False):
        ...
        super(Grace_flip, self).__init__()
        ...
        for i in range(train_iters):
            ...
            for j in range(num_batches):
                ...
                if(self.if_smoothed==True):
                    ...
                    if(self.args.if_ignore_structure2 == True):
                        edge_index_2,edge_weight_2 = self.sample_noise_all_sparse(self.args,edge_index_2,edge_weight_2,self.features)
    ...
    def _train_without_val(self, labels, train_iters, verbose):
        ...
        for i in range(train_iters):
            ...
            edge_index, edge_weight = self.edge_index.clone().detach(), None
            # edge_index_1,x_1,edge_weight_1,edge_index_2,x_2,edge_weight_2 = construct_augmentation_1by1(self.args, self.features, edge_index, edge_weight)
            ...
            #     elif(self.args.if_keep_structure1 == False):
            #         # edge_index_1,edge_weight_1 = self.sample_noise_all(edge_index_1,edge_weight_1)
            #         edge_index_1,edge_weight_1 = sample_noise_all_dense(self.args,edge_index_1,edge_weight_1,self.device)
            #         # edge_index_1,edge_weight_1 = self.sample_noise_all_sparse(self.args,edge_index_1,edge_weight_1,self.features)
            #     if(self.args.if_ignore_structure2 == True):
            #         edge_index_2,edge_weight_2 = sample_noise_all_dense(self.args,edge_index_2,edge_weight_2,self.device)
            #         # edge_index_2,edge_weight_2 = self.sample_noise_all_sparse(self.args,edge_index_2,edge_weight_2,self.features)
            #     # edge_index_1,edge_weight_1 = self.sample_noise_all(edge_index_1,edge_weight_1,idx_train)
            #     # edge_index_2,edge_weight_2 = self.sample_noise_all(edge_index_2,edge_weight_2,idx_train)
            #     # idx_overall = torch.tensor(range(self.features.shape[0])).to(self.device)
            ...
            # edge_index_1 = pyg_utils.dropout_adj(edge_index, p=self.args.drop_edge_rate_1)[0]
            # edge_index_2 = pyg_utils.dropout_adj(edge_index, p=self.args.drop_edge_rate_2)[0]
            # x_1 = drop_feature(self.features, self.args.drop_feat_rate_1)
            # x_2 = drop_feature(self.features, self.args.drop_feat_rate_2)  

            adj_1 = pyg_utils.to_dense_adj(edge_index_1,max_num_nodes=self.features.shape[0])[0]
            adj_2 = pyg_utils.to_dense_adj(edge_index_2,max_num_nodes=self.features.shape[0])[0]
            # adj_1 = edge2adj(x_1, edge_index_1)
            # adj_2 = edge2adj(x_2, edge_index_2)
            if(self.args.if_keep_structure1 == True): 
                pass
                adj_1_noise = adj_1
            else:
                adj_1_noise = adj_1.clone().detach()
                m = Bernoulli(torch.tensor([self.args.prob]).to(self.device))
                
                mask = m.sample(adj_1.shape).squeeze(-1).int()    
                rand_inputs = torch.randint_like(adj_1, low=0, high=2).squeeze().int().to(self.device)
                
                adj_1_noise = adj_1 * mask + rand_inputs * (1 - mask)
                for idx in range(adj_1.shape[0]):
                    adj_1_noise[idx, idx] = adj_1[idx, idx]
                    adj_1_noise[:,idx] = adj_1_noise[idx]

            z1 = self.forward(x_1, adj_1_noise)

            z2 = self.forward(x_2, adj_2)
        ...
        for i in range(train_iters):
            ...
            if(self.if_smoothed==True):
                ...
                if(self.args.if_keep_structure1 == True): # or ignore_structure or random_drop
                    ...
                    edge_index_1,edge_weight_1 = self.sample_noise_all_sparse(self,self.args,edge_index_1,edge_weight_1,self.features)
                if(self.args.if_ignore_structure2 == True):
                    edge_index_2,edge_weight_2 = self.sample_noise_all_sparse(self,self.args,edge_index_2,edge_weight_2,self.features)
                # edge_index_1,edge_weight_1 = self.sample_noise_all(edge_index_1,edge_weight_1,idx_train)
                # edge_index_2,edge_weight_2 = self.sample_noise_all(edge_index_2,edge_weight_2,idx_train)
                # idx_overall = torch.tensor(range(self.features.shape[0])).to(self.device)
                # edge_index_1,edge_weight_1 = self.sample_noise_1by1(edge_index_1, edge_weight_1,idx_overall)
